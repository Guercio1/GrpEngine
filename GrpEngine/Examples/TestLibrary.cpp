...
void TestLibraryGest(void)
{
	...
	egtkSetColour(RGB565_YELLOW, RGB565_BLUE);

	egtkSetFont(&FONT_32X53,1, 1, 1);
	egtkSetPos(egtkGetScreenW()/3, egtkGetScreenH()/2);
	egtkWriteString((char*)"Test",false);

	egtkSetPos(egtkGetScreenW() / 3, egtkGetScreenH()*2 / 3);
	egtkWriteString((char*)"Test", true);
	...
	egtkDrawGlyph(egtkGetScreenW() / 2, 0, RGB565_YELLOW, RGB565_BLUE, &Elcos, true);
	egtkDrawFitGlyph(egtkGetScreenW()/2+Elcos.width, 0, Elcos.width / 2, Elcos.height / 2, RGB565_YELLOW, RGB565_BLUE, &Elcos);
